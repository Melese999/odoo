...
class MrpProduction(models.Model):
    ...
    _index = 'sale_line_id_index'  # Note: _index is not a standard Odoo field property

    # --- Custom Manufacturing Dimensions (Float/Manual Fields) ---
    length = fields.Float(
        string="Unit Length (m)",
        digits='Product Unit of Measure',
        help="Actual length per unit in meters, received from the sales order."
    ...
    # The original 'pitch' field (Float) is replaced by the related field (Selection).
    # If you still need the float, rename it (e.g., 'pitch_float').
    # For now, I'm defining the new related field with the name 'pitch'.
    # If you need the old float field, uncomment it and rename it.

    # pitch = fields.Float(  # <-- Original field definition
    #     string="Pitch (Float)",
    #     help="The pitch value for tile products, received from the sales order."
    # )

    # --- Inherited Fields from Sale Order (via sale_line_id) ---

    # 1. Related Pitch (Selection)
    # Path: sale_line_id (SOL) -> order_id (SO) -> pitch (field on SO)
    pitch = fields.Selection(
        related='sale_line_id.order_id.pitch',
        string='Pitch (SO)',
        readonly=True,
        store=True,
        help="The pitch value selected on the Sale Order."
    )

    # 2. Related Effective Width (Selection)
    '''effective_width = fields.Selection(
        related='sale_line_id.order_id.effective_width',
        string='Effective Width (SO)',
        readonly=True,
        store=True,
        help="The effective width selected on the Sale Order."
    )'''

    # 3. Related Production Priority (Selection)
    production_priority = fields.Selection(
        related='sale_line_id.order_id.production_priority',
        string='Production Priority',
        readonly=True,
        store=True,
        help="The production priority selected on the Sale Order."
    )
    ...
    def _compute_total_dimensions(self):
        """Compute total dimensions based on quantity"""
        for production in self:
            # Note: The usage of 'pitch' here will now refer to the related Selection field (string key)
            # You must adapt your calculation if it needs the old float value.
            production.total_length = production.length * production.product_qty
            production.total_weight = production.weight * production.product_qty
