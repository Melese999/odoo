...
class CommissionBill(models.Model):
    ...
    def action_pay(self):
        ...
        for bill in self:
            ...
            # Trigger summary generation for monthly paid bills
    ...
    #     for bill in self:
    #         # Only process if bill is paid and not already in a summary
    #         if bill.state != self.PAID or bill.summary_id:
    #             continue
    #
    ...
    #
    #         # Only create summary if we have at least one paid bill
    #         if paid_bills:
    #             summary = self._create_monthly_summary(paid_bills, month_start, month_end)
    #             if summary:
    #                 # Update all bills to completed state and link to summary
    #                 paid_bills.write({
    ...
    #                     "Summary generated with %d bills for period %s to %s"
    #                 ) % (len(paid_bills), month_start, month_end))
        ...
        for bill in self:
            ...
            if paid_bills:
                ...
                if summary:
                    ...
                    # Log the summary generation
                    summary.message_post(body=_(
                        "Summary generated with %d bills and %d commission records for period %s to %s"
                    ) % (
                                                  len(paid_bills),
                                                  sum(paid_bills.mapped(lambda b: len(b.commission_records))),
                                                  month_start,
                                                  month_end
                                              ))
    ...
    def _create_monthly_summary(self, bills, month_start, month_end):
        """Create a monthly bill summary with unique name"""
        try:
            # Generate base name
            date_range = f"{month_start.strftime('%Y-%m-%d')}-{month_end.strftime('%Y-%m-%d')}"
            base_name = f"Bill-Summary-{date_range}"

            # Check for existing summaries with same name and append unique identifier if needed
            existing = self.env['commission_system.bill.summary'].search_count([
                ('name', '=like', f"{base_name}%")
            ])

            if existing:
                summary_name = f"{base_name}-{uuid.uuid4().hex[:8].upper()}"
            else:
                summary_name = base_name

            # Create the summary - MAKE SURE TO SET THE NAME FIELD
            summary = self.env['commission_system.bill.summary'].create({
                'name': summary_name,  # This was missing!
            ...
            self.message_post(body=_("Failed to generate monthly summary: %s") % str(e))
            return None
    ...
    def _validate_state_transition(self, new_state):
        ...
        allowed_transitions = {
            ...
            self.PAID: [self.COMPLETED, self.AUDITED],  # Updated
...
class CommissionBillSummary(models.Model):
    ...
    # New computed field to get all commission records from all bills
    commission_record_ids = fields.One2many(
        'commission_system.records',
        string='Commission Records',
        compute='_compute_commission_records'
    )
    commission_record_count = fields.Integer(
    ...
    @api.depends('bill_ids', 'bill_ids.commission_records')
    def _compute_commission_records(self):
        """Compute all commission records from all bills in this summary"""
        for summary in self:
            all_records = self.env['commission_system.records']
            for bill in summary.bill_ids:
                all_records += bill.commission_records
            summary.commission_record_ids = all_records
            summary.commission_record_count = len(all_records)

    @api.depends('bill_ids.total_commission', 'bill_ids.total_tax', 'bill_ids.net_commission')
    def _compute_totals(self):
        for summary in self:
            summary.total_commission = sum(summary.bill_ids.mapped('total_commission'))
            summary.total_tax = sum(summary.bill_ids.mapped('total_tax'))
            summary.total_net_commission = sum(summary.bill_ids.mapped('net_commission'))
            summary.bill_count = len(summary.bill_ids)
    ...
    def create(self, vals):
        ...
        summary.message_post(body=_("Bill summary created for period %s") % summary.date_range)
        return summary

    def action_view_commission_records(self):
        """View all commission records from all bills in this summary"""
        self.ensure_one()
        return {
            'name': _('Commission Records - %s') % self.name,
            'type': 'ir.actions.act_window',
            'res_model': 'commission_system.records',
            'view_mode': 'tree,form',
            'domain': [('id', 'in', self.commission_record_ids.ids)],
            'context': {
                'create': False,
                'edit': False,
            },
